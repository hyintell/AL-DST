...
class DSTMultiWozData:
    def __init__(self, model_name, tokenizer, data_path_prefix, shuffle_mode='shuffle_session_level', 
        data_mode='train', add_prefix=True, add_special_decoder_token=True, train_data_ratio=1.0, 
                 only_last_turn=False, only_random_turn=False, pre_selected_turn_ids=[]):
        '''
        ...
        if data_mode == 'train':
            ...
            self.train_data_id_list = self.tokenize_raw_data(train_raw_data)
            
            # use pre_selected_dialogue_ids 
            if len(pre_selected_turn_ids) != 0:
                # extract dialogue id
                pre_selected_dialogue_ids = [turn_id.split('-')[0].lower() for turn_id in pre_selected_turn_ids]
                self.train_data_id_list = [session for session in self.train_data_id_list if session[0]['dial_id'] in pre_selected_dialogue_ids]
                print(f'self.train_data_id_list: {len(self.train_data_id_list)}')
#                 print_list = [session[0]['dial_id'] for session in self.train_data_id_list if session[0]['dial_id']]
#                 print(f'self.train_data_id_list: {print_list}')
                
            
#             # change: TEST for large turns
#             my_list = ['mul1191', 'mul1332', 'mul1353']
#             self.train_data_id_list = [session for session in self.train_data_id_list if session[0]['dial_id'] in my_list]
#             print(f'self.train_data_id_list: {len(self.train_data_id_list)}')
            
            self.train_data_list = self.flatten_data(self.train_data_id_list, only_last_turn=only_last_turn, only_random_turn=only_random_turn)
...
#         self.dev_data_list = self.flatten_data(dev_data_id_list, only_last_turn=False)
        print('Use only last turn of Validation set to validate...')
        self.dev_data_list = self.flatten_data(dev_data_id_list, only_last_turn=True)
        ...
        #### DEBUG: use a few data for testing
#         print(f'*********** use a few data for testing ***********')
# #         self.train_data_list = self.train_data_list[:100]
#         self.dev_data_list = self.dev_data_list[:100]
#         self.test_data_list = self.test_data_list[:100]

#         ###### Low Resource: K
#         print(f'before random select 500 last turns: {len(self.train_data_list)}')
#         random.shuffle(self.train_data_list)
#         self.train_data_list = self.train_data_list[:500]
#         print(f'after random select 500 last turns: {len(self.train_data_list)}')
...
#             print(f'len self.train_data_list: {len(self.train_data_list)}')
#             print(self.train_data_list[:5])
        else:
            pass
        
    ################ new functions ############################
    
    def get_selected_turn_ids(self):
        self.selected_turn_ids = []
        for data_dict in self.train_data_list:
            self.selected_turn_ids.append(f"{data_dict['dial_id'].upper()}-{data_dict['turn_num']}")
        
        return self.selected_turn_ids
    
    
    def get_dial2len(self):
        """
        Get dialogue ID and its length(number of turns)
        """
        
        dialogue_id_list = []
        total_turns_list = []
        for session in self.train_data_id_list:
            dialogue_length = len(session)
            dialogue_id = session[0]['dial_id'].upper()
            dialogue_id_list.append(dialogue_id)
            total_turns_list.append(dialogue_length)
            
        df = pd.DataFrame({
            'dialogue_id': dialogue_id_list,
            'total_turns': total_turns_list
         })
        
        return df
    
    
    def get_dialogue_id_list(self):    
        dialogue_id_list = []
        for session in self.train_data_id_list:
            dialogue_id = session[0]['dial_id']
            dialogue_id_list.append(dialogue_id)
            
        return dialogue_id_list
    
    
    def save_csv(self, df, save_path, filename):
        """
        ...
        save_result_prefix = f'{save_path}/{filename}'
    ...
    def flatten_data(self, data, only_last_turn=False, only_random_turn=False):
        '''
        ...
        for session in data:
            ...
            ## CHANGE: random select a turn index based on the length
            if only_random_turn:
                random_turn_index = np.random.randint(turn_num)
#                 print(f'turn_num: {turn_num} random_turn_index: {random_turn_index}')
            ...
            for turn_id in range(turn_num):
                ...
                ## CHANGE: only use the last turn of a dialogue
                if only_last_turn:
                    if turn_id == turn_num - 1:
                        ...
                        pass
                elif only_random_turn:
                    if turn_id == random_turn_index:
                        data_list.append({'dial_id': one_dial_id,
                            'turn_num': turn_id,
                            'prev_context':previous_context,
                            'user': curr_turn['user'],
...
#             self.ordering_train_data()
    ...
    def build_batch_list(self, all_data_list, batch_size):
        ...
        for i in range(batch_num):
            ...
            #### CHANGE: +1 to cover the last item
...
#             for idx in range(start_idx, end_idx):
    ...
    ################ new functions ############################
    def get_dialogue_turns_by_id(self, dialogue_id):
        return [instance for instance in self.train_data_list if instance['dial_id'] == dialogue_id]
    
    def get_turn_instances_by_id(self, total_selected_turn_ids):

        return [instance for instance in self.train_data_list \
                if f"{instance['dial_id'].upper()}-{instance['turn_num']}" in total_selected_turn_ids]
    
    def build_iterator_by_selected_turns(self, total_selected_turn_ids, batch_size):

        batch_list = []
        all_data_list = self.get_turn_instances_by_id(total_selected_turn_ids)
...
#         print(f'all_data_list: {all_data_list}')
        ...
        for i in range(batch_num):
            ...
            batch_list.append(one_batch)
        out_str = 'Overall Number of datapoints is ' + str(data_num) + \
        ' Number of batches is ' + str(len(batch_list))
        print (out_str)
#         print (batch_list)
    ...
    def build_AL_batch_list(self, dialogue_turns, eva_batch_size=24):
        data_list = dialogue_turns
