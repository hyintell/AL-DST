...
class T5Gen_Model(nn.Module):
    ...
    def batch_generate(self, src_input, src_mask, generate_mode, max_decode_len):
        ...
        outputs_dict = self.model.generate(input_ids = src_input, attention_mask = src_mask, decoder_start_token_id = start_token_id,
        ...
        B = outputs_dict.sequences.shape[0]
        generated_output_entropy = torch.zeros(B).to(src_input.device)
        max_predicted_val_list = torch.zeros(B).to(src_input.device)

        # add the tuple of all scores together
        for scores in outputs_dict.scores:
            next_entropy = Categorical(logits=scores).entropy()
            generated_output_entropy += next_entropy
...
#         print(self.tokenized_decode(src_input[0]))
        
#         print(f'max_predicted_val_list: {max_predicted_val_list.shape}')
...
#             print (one_res_text)
        ...
        return res_text_list, generated_output_entropy, max_predicted_val_list
