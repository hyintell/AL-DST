...
def batch_generate(model, one_inference_batch, data):
    ...
    batch_bs_text, generated_output_entropy, max_predicted_val_list = model.batch_generate(bs_tensor, bs_mask, generate_mode='bs', max_decode_len=max_response_len)
    for idx in range(batch_size):
        one_bs_text = batch_bs_text[idx]
        res_batch_parse_dict[idx]['bspn_gen'] = one_bs_text
    return res_batch_parse_dict, generated_output_entropy, max_predicted_val_list
