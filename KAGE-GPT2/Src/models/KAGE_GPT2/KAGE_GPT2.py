...
#             print('skip reset!')
    ...
    def graph_forward(self, ds_embeddings, cls_labels=None):
        ...
        if self.sys_config.model_config.graph_mode == 'full':
            ...
            # create new ontology - connect domain-slots S = 30 * 30
...
#         assert input_ids.shape[0] == 1  # batch size 1
        ...
        # get the toal sum of the generated token's entropy
        # allow Batch generation instead of fix batch size to 1
        B = input_ids.shape[0]
#         print(f'B: {B}')
        generated_output_entropy = torch.zeros(B).to(input_ids.device)
        max_predicted_val_list = torch.zeros(B).to(input_ids.device)
        
        count = torch.zeros(B).to(input_ids.device)
        flag = -1
        while cur_len < max_length:
            ...
            count = (input_ids[:, -1] == sep_token_id) + count

            ## for each batch position, we check if the last token is <BOS> or <SEP> or <PAD>
            ## if not, then the token is a value token, then we add the graph enhanced ds embeddings to zero-tensor
            for i in range(B):
                if input_ids[i, -1] != bos_token_id and input_ids[i, -1] != sep_token_id and input_ids[i, -1] != pad_token_id:
                    count_i = int(count[i])
                    if count_i < len(ds_embeddings):
                        cat_hidden_states[i, -1, :] = ds_embeddings[count_i][i]
                    else:
                        cat_hidden_states[i, -1, :] = ds_embeddings[-1][i]
            ...
            #### calculate the generated token's entropy #### 
            next_token_entropy = Categorical(logits=scores).entropy()
            generated_output_entropy += next_token_entropy

            # argmax
            next_tokens = torch.argmax(scores, dim=-1)
            
            #### calculate the confidence #### 
            max_predicted_val = torch.max(scores, dim=-1)[0]
            max_predicted_val_list += max_predicted_val
            
            for i in range(B):
                if count[i] >= len(ds_ids):
                    if next_tokens[i] == sep_token_id:
        ...
        return input_ids, generated_output_entropy, max_predicted_val_list
    ...
    def refresh_embeddings(self):
        ...
        for index, text in self.ontology_value_id2tokenized_text.items():
            ...
            embeddings = self.embedding_layer(ids) # size: x * 768, x can be 1,2,3... depends on the number of token ids
            # TODO Embedding aggregatioon
            agg_embeddings = torch.sum(embeddings, dim=0)  # after sum on dim=0, 768
    ...
    def add_KB(self,
        ...
        ## UPDATE: init an adjacency matrix that connects each ds node with its all possible values
        if self.sys_config.model_config.graph_mode == 'full' or self.sys_config.model_config.graph_mode == 'include_previous_state':
            print('Initializing the full graph...')
            ...
            print(f'Adjacency matrix S: {self.S}')
