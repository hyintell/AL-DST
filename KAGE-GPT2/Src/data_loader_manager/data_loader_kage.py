...
class DataLoaderWrapper():
    ...
    def set_dataloader(self, config, tokenizer, data_type, run_type,
                       value_id2text, value_text2id, ds_list, ds_text2id, data_size=-1, selected_dialogue_ids=None, selected_turn_ids=None):
        ...
        dataset = Dataset(config, tokenizer, data_type, run_type == 'generation',
                          ...
                          self.SPECIAL_TOKENS, self.SPECIAL_TOKENS_VALUES, data_size, selected_dialogue_ids=selected_dialogue_ids, selected_turn_ids=selected_turn_ids)
...
#             sampler = SequentialSampler(dataset)
        if data_type == 'dev' or data_type == 'test' or data_type == 'train_select_dialogue':
            sampler = SequentialSampler(dataset)  # if args.local_rank == -1 else DistributedSampler(train_dataset)
            print('Using Sampler: sequential')
        else:
            sampler = RandomSampler(dataset)
            print('Using Sampler: random')
        
        ###### use SequentialSampler no matter it is train/dev/test
#         sampler = SequentialSampler(dataset)
...
#         elif data_type == 'valid':
        ...
        elif data_type == 'train_select_dialogue':
            batch_size = config.train.train_select_dialogue_batch_size
        elif data_type == 'train_select_turn':
            batch_size = config.train.train_select_turn_batch_size
            
        print(f'$$$ data_type: {data_type} batch_size: {batch_size}')
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config, tokenizer, data_type, generation,
                 ...
                 data_size, selected_dialogue_ids=None, selected_turn_ids=None):
#         print(f'@@@@@@@@@@@@ generation: {generation}')
#         assert data_type in ['train', 'dev', 'test', 'demo']
        self.selected_dialogue_ids = selected_dialogue_ids
        self.selected_turn_ids = selected_turn_ids
        self.config = config
        ...
        # create unlabelled examples for turn selection
        if data_type == 'train_select_dialogue' or data_type == 'train_select_turn':
            self._create_selected_examples()
        else:
    ...
    def _process_dst(self, split):
        ...
        dst_files = {
            ...
            'train_select_dialogue': self.config.data_loader.additional.train_data_path,
            'train_select_turn': self.config.data_loader.additional.train_data_path,
        }
        dst_f = dst_files[split]
#         print(f'$$$: file_path: {dst_f}')
        ...
        dst_cont, dial_n, example_n, dialogue_id2turns, dialogue_id_list, _ = iterate_dst_file(dst_f)
#         print('{} -> # of dialogues: {}, examples: {}'.format(split, dial_n, example_n))
        self.dst_data = dst_cont
        self.dialogue_id2turns = dialogue_id2turns
        print(f'\n ==> {split} | dial_n: {dial_n} | example_n: {example_n}')
...
#         ### UPDATE: for testing, for 'dev', also load only last turn
#         if self.data_type == 'dev' and self.generation:
#             load_only_last_turn = True
...
#                 load_only_last_turn = False
                load_only_last_turn = True
                print('Loading only last turn for Valid set')
#                 print('Loading all turns for validation [testing mid turn performance]...')
        ...
        ## UPDATE: for valid set, only use the last turn to save time
        if self.config.mode == 'select_turn' and self.data_type == 'dev' and self.generation: 
            load_only_last_turn = True
            print('Loading only last turn for Valid set')

        slot_label_counter = {}
        
        # $$$ Actual total examples: 54971
#         print(f'Actual total examples: {self.data_type} {len(self.dst_data)}')
        print(f'selected_dialogue_ids: {self.selected_dialogue_ids}')
        print(f'self.selected_turn_ids: {self.selected_turn_ids}')
        for example_num, example_id in enumerate(sorted(self.dst_data.keys())):  
#         for example_num, example_id in enumerate(tqdm(sorted(self.dst_data.keys()))):   
            if self.data_size != -1 and example_num == self.data_size:
                break
            ## example_num: range from [0, 54971), example_id: MUL0417-1, MUL0417-2, MUL0417-3, ... it is the turn index
#             print('=====')
...
#             print(self.dst_data[example_id]['current_turn_index'], self.dst_data[example_id]['total_num_turn'])
            ...
            if self.selected_turn_ids is not None:
                if example_id not in self.selected_turn_ids:
                    continue
            else:
                if load_only_last_turn:
#                     # Use only last turn + random turn
# #                     if self.dst_data[example_id]['current_turn_index'] < self.dst_data[example_id]['total_num_turn'] - 1 and self.dst_data[example_id]['current_turn_index'] != self.dst_data[example_id]['random_turn_index']:
#                     if self.dst_data[example_id]['current_turn_index'] != self.dst_data[example_id]['random_turn_index']:
# #                         print(self.dst_data[example_id]['current_turn_index'], self.dst_data[example_id]['random_turn_index'], self.dst_data[example_id]['total_num_turn'])
# #                         print('skip')
#                         continue
...
#                 print(input_ids)
            ...
            self.examples.append({
                ...
                'random_turn_index': self.dst_data[example_id]['random_turn_index'],
                'current_turn_index': self.dst_data[example_id]['current_turn_index'],
                'dialogue_id': self.dst_data[example_id]['dialogue_id']
                # 'str_ds_pair': [str_ds_pair for str_ds_pair, ds_id in self.ds_text2id.items()]
            })
#         print(f'+++ number of examples: {len(self.examples)}')
        
#         import collections
#         unique_counts = collections.Counter(e['dialogue_id'] for e in self.examples)
#         print(f'+++ unique_counts: {len(unique_counts.keys())}')
            
        ## TESTTING:: select 20 examples
        if self.data_type == 'train':
            # random sample a few examples
            print('randomly select 500 instances for training')
            random.shuffle(self.examples)
            self.examples = self.examples[:500] 
            print(f'self.examples len: {len(self.examples)}')
...
#             print(self.examples[1])
    ...
    def _create_selected_examples(self):
        """
        This function create unlabelled examples based on which dialogues needs to be extracted

        :self.dialogue_id2turns: dict, key: dialogue id, value: a list of dict, each is a turn, total has 7888
        :selected_dialogue_ids: np array, list of dialogue_id that are selected
        """
        self.examples = []
        slot_label_counter = {}
        
        print(f'selected_dialogue_ids: {self.selected_dialogue_ids}')
        print(f'selected_turn_ids: {self.selected_turn_ids}')
        
        for dialogue_id, turn_list in sorted(self.dialogue_id2turns.items()):
#         for dialogue_id, turn_list in tqdm(sorted(self.dialogue_id2turns.items())):
            
            # skip if not the selected dialgoue
            if self.selected_dialogue_ids is not None and dialogue_id not in self.selected_dialogue_ids:
                continue
            
            # each turn in the dialogue
            for turn_item in turn_list:
                # example_id = 'MUL001-0'
                example_id = turn_item['example_idx']
                
                # skip if not the selected turn
                if self.selected_turn_ids is not None and example_id not in self.selected_turn_ids:
                    continue
                ...
                # default self.config.use_graph == 1
                if self.config.use_graph:
                    # dialogue_context <BOS> d1 s1 <SEP> d2 s2 <SEP> d3 s3 <EOS>
                    # pre_input_ids = context_ids + [self.bos_id] + pre_target_ids + [self.eos_id]
                    # dialogue_context <BOC> d1 s1 <CLS> d2 s2 <CLS> d3 s3 <CLS>
                    pre_input_ids = context_ids + [self.boc_id] + pre_target_ids + [self.cls_id] # pre_input_ids does not contain true slot value token ids
                    if len(pre_input_ids) >= 1024:
                        pre_input_ids = pre_input_ids[-1023:]
                    pre_ignore_len = len(pre_input_ids) - len(pre_target_ids) - 1 
                    pre_ds_indice = [] 
...
#             print(self.examples[0])
    ...
    def collate_fn(self, batch):
        ...
        random_turn_index = [ex['random_turn_index'] for ex in batch]
        current_turn_index = [ex['current_turn_index'] for ex in batch]
        dialogue_id = [ex['dialogue_id'] for ex in batch]
        return {
            'pre_input_ids': pre_input_ids,
            ...
            'example_id': example_id,
            'turn_utt': turn_utt,
            'random_turn_index': random_turn_index,
