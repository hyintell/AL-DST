...
# load the slot_list.json file based on ENV
env = os.getenv('ENV')
if env == 'sagemaker_training_job':
    # running on Sagemaker remote
    slot_list_filepath = '../input/data/training/MultiWOZ/dst/slot_list.json'
else:
    # running on local mode
    slot_list_filepath = '../Data/MultiWOZ/dst/slot_list.json'
    
with open(slot_list_filepath) as f:
	all_slots = json.load(f)
...
def iterate_dst_file(file_path):
	...
	for dial_dict in dst_data:
		...
		# ########## for each dialogue, generate a random turn index to pick ############
        # # note to use length - 1 becasue we don't want to use the last turn index
		# if len(dial_dict["dialogue"]) - 1 == 0:
		# 	random_turn_index = -1
		# else:
		# 	random_turn_index = np.random.randint(len(dial_dict["dialogue"]) - 1)
		# print(f'total length: {len(dial_dict["dialogue"])} random_turn_index: {random_turn_index}')

		########## for each dialogue, generate a random turn index to pick ############
        # allow the last turn
		random_turn_index = np.random.randint(len(dial_dict["dialogue"]))
		# print(f'total length: {len(dial_dict["dialogue"])} random_turn_index: {random_turn_index}')

		turn_list = []
		dialogue_id_list.append(dial_name.replace('.json', ''))
		dialogue_id2len[dial_name.replace('.json', '')] = len(dial_dict["dialogue"])
		for turn_idx, turn in enumerate(dial_dict["dialogue"]):
			...
			data_container[example_idx]['random_turn_index'] = random_turn_index # int
			data_container[example_idx]['dialogue_id'] = dial_name.replace('.json', '')  # the id of the dialogue that current utterance belongs to

			turn_list.append({
				'example_idx': example_idx,
				'current_turn_index': turn_idx,
				'context': history,
				'turn_utt': turn_utterance,
				'belief_state': bs_dict,
				'total_num_turn': len(dial_dict["dialogue"]),
				'random_turn_index': random_turn_index
			})
		dialogue_id2turns[dial_name.replace('.json', '')] = turn_list

	# len(dst_data): number of dialogue dicts in the json file
	# len(data_container): number of concatenated sys and usr utterances of each turn
	dial_n, example_n = len(dst_data), len(data_container)
	return data_container, dial_n, example_n, dialogue_id2turns, dialogue_id_list, dialogue_id2len
